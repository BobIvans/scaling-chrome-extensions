...
test('client returns binary artifact bytes without UTF-8 decoding',async()=>{const {createHash}=await import('node:crypto');const bytes=Buffer.from([0,255,128,1]);const c=new NativeClient(port(m=>{assert.equal(m.type,'artifact');assert.equal(m.artifactId,'file-id');return {bytes:4,offset:0,sha256:createHash('sha256').update(bytes).digest('hex'),base64:bytes.toString('base64')};}));assert.deepEqual(Buffer.from(await c.resultBytes('job','file-id')),bytes);c.close();});
