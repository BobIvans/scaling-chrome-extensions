...
$('recent-load').onclick=async()=>{const token=epoch;try{const {captures}=await recentRequest('getRecent');const additions=[];for(const c of captures){if(records.some(r=>r.captureId===c.captureId))continue;additions.push({id:crypto.randomUUID(),captureId:c.captureId,name:'Снимок '+c.capturedAt,text:c.text,source:c.source||'Источник не указан',createdAt:c.capturedAt,status:c.status,warnings:c.warnings||[],sha256:await digest(encode(c.text))});}if(token!==epoch)throw Error('Библиотека удалена; поздний ответ отклонён.');validate([...records,...additions]);records.push(...additions);for(const r of additions)selected.add(r.id);render();notice('Добавлено недавних снимков: '+additions.length+'. Для восстановления после перезапуска сохраните библиотеку.');}catch(e){notice(e.message);}};
$('recent-download').onclick=async()=>{try{await recentRequest('downloadRecent');notice('Скачивание общего TXT сессии запрошено. Завершение проверьте в Chrome.');}catch(e){notice(e.message);}};
$('recent-clear').onclick=async()=>{try{if(!confirm('Очистить временную память собранных снимков? Уже добавленные копии в библиотеке, последний снимок OCC и скачанные файлы сохранятся.'))return;await recentRequest('clearRecent');notice('Память снимков сессии очищена. Копии в библиотеке не удалены.');}catch(e){notice(e.message);}};
