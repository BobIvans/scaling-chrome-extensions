import {encode} from './convert.mjs';
import {aggregateWorkspace,normalizedRecord} from './workspace.mjs';
...
function safeRecords(){
  ...
  if(!saved||![1,2].includes(saved.version)||!Array.isArray(saved.records))return [];
  try{return saved.records.map(r=>normalizedRecord(r));}catch{return [];}
}
...
$('build').onclick=async()=>{
  try{
    ...
    const text=aggregateWorkspace(chosen);bundle=await splitBundle(text,{partBytes:Number($('limit').value),label:'occ_saved_library'});current=0;showPart();
