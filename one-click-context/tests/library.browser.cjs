...
test('real browser agent file download with mocked native API rejects synthetic connect',async()=>{
 ...
 await p.goto(base+'/library.html');await p.locator('summary').filter({hasText:'Задания локальному Codex'}).click();await p.evaluate(()=>document.querySelector('#agent-connect').click());assert.deepEqual(await p.evaluate(()=>nativeCalls),[]);await p.locator('#agent-connect').click();await p.waitForFunction(()=>document.querySelector('#agent-mode option[value=build]').disabled);assert.equal(await p.locator('#agent-mode option[value=build]').evaluate(el=>el.disabled),true);await p.locator('#agent-disconnect').click();await p.evaluate(()=>globalThis.fixtureBuild=true);await p.locator('#agent-connect').click();await p.waitForFunction(()=>!document.querySelector('#agent-mode option[value=build]').disabled);assert.equal(await p.locator('#agent-mode option[value=build]').evaluate(el=>el.disabled),false);assert.match(await p.locator('#agent-mode option[value=build]').textContent(),/запись внутри папки/);await p.getByRole('button',{name:'Файлы результата',exact:true}).click();const event=p.waitForEvent('download');await p.getByRole('button',{name:'Скачать report.zip · 4 байт',exact:true}).click();assert.deepEqual(fs.readFileSync(await(await event).path()),bytes);await p.close();
});

test('recent UI imports once with source dates and requests aggregate download',async()=>{
 const p=await context.newPage();
 await p.addInitScript(()=>{globalThis.recentCalls=[];window.chrome={runtime:{id:'fixture',sendMessage:async m=>{recentCalls.push(m.type);return {ok:true,captures:[{captureId:'fixture-id',capturedAt:'2026-08-01T12:00:00.000Z',source:'https://example.org/chat',text:'Привет',status:'PARTIAL',warnings:['time limit']}]};}}};});
 await p.goto(base+'/library.html');await p.locator('#recent-load').click();await p.waitForFunction(()=>document.querySelectorAll('.item').length===1);
 await p.locator('#recent-load').click();await p.waitForFunction(()=>document.querySelector('#status').textContent.includes('снимков: 0'));
 assert.equal(await p.locator('.item').count(),1);assert.match(await p.locator('#list').textContent(),/2026-08-01/);assert.match(await p.locator('#preview').inputValue(),/time limit/);
 assert.equal(await p.evaluate(()=>localStorage.getItem('occ-library-v1')),null);
 await p.locator('#recent-download').click();assert.ok((await p.evaluate(()=>recentCalls)).includes('downloadRecent'));
 await p.close();assert.equal(await page.locator('#recent-download').isDisabled(),true);
});
